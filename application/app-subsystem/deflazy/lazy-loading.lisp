...
(defun create-env (&optional (tree nil) (old-nicknames (list (nickname-foo (tree-name tree) nil)))
		     (nicknames-names
		      (mapcar (lambda (x)
				(cons (cdr x) (car x)))
...
(defun test23 ()
  (reset-enanv)
  (getfnc 'test2 :env *enanv*))
(defun another-test ()
  ;;Multiply instantiated deflazy
  (let ((*tree* `(()
		  ((test2 . top)
		   ((test0 . test0)
		    ((quux . quux-0)))
...
(defun another-test2 ()
  ;;Multiply instantiated deflazy
  ;;one env referencing another env
  (let* ((env (create-env))
	 (*tree* `(()
		   ((test2 . top)
		    ((test0 . test0)
